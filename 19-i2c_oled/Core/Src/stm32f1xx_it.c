...
//   {
//     HAL_UART_Transmit(huart, Rx1_Buf, 4, HAL_MAX_DELAY); // 发送接收到的数据
//     HAL_UART_Receive_IT(&huart1, Rx1_Buf, 4);            // 再开启接收中断
//   }
// }
...
void USER_UART_IRQHandler(UART_HandleTypeDef *huart)
{
  if (huart->Instance == USART1) // 判断是否是串口1
  {
    if (__HAL_UART_GET_FLAG(huart, UART_FLAG_IDLE) != RESET) // 判断是否是空闲中断
    {
      ...
      // 计算接收到的数据长度：BUFFERSIZE-HAL_DMA_GET_COUNTER(&hdma_USart1_rx)
